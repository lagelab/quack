...
getConnectivityInfoForPathwayList <- function(phash){
  ...
  wholeSteps <- floor(upperBound/stepSize)
  remainder <- upperBound %% stepSize
  
  stepList <- rep(stepSize, wholeSteps)
  if(remainder>0) stepList <- c(stepList,remainder)
  
  infoFile <- data.frame()
  
  for(s in 1:length(stepList)){
    
    start_i <- j
    end_i <- j + (stepList[s]-1)
    
    r <- foreach(i = start_i:end_i,
                 .export=ls(envir=globalenv()),
                 .packages=c("hash","igraph") ) %dopar% {
...
GetQuackModelingFile <- function(genesetHash,inSamplePathways, logOn=TRUE,stepSize=50,normMetrics){
  ...
  for(s in 1:length(stepList)){
    ...
    #change back to dopar
    r <- foreach(i = start_i:end_i,
                 .export=ls(envir=globalenv()),
                 .packages=c("hash","igraph","pROC")) %dopar% { 
    ...
    if(logOn){
      print(paste("% complete: ", round(end_i/upperBound,2), sep=""))
    }
...
#' @export
GetQuackClassifier <- function(response, predictors, numberOfTrees=500, withReplacement,fileName, saveResults, varImportDir,geneModDir){
...
bootstrapAUC_tm <- function(aucFile, stepSize, bootSampleSize,samplePerc){
  ...
  j <- 1
  while (j <= bootSampleSize){
    
    start_i <- j
    end_i <- j + stepSize
    
    #adjust end i if needed
    if(end_i>=bootSampleSize){
      end_i <- bootSampleSize
    }
    
    r <- foreach(i = start_i:end_i,
                 .export=ls(envir=globalenv()),
                 .packages=c("randomForest","igraph","pROC")) %dopar% {
                   values <-  sample(indexList,round(samplePerc*numObs),replace = TRUE)
                   thisDS <- aucFile[values,]
                   thisAUC <- NA
                   #try(thisAUC <- as.numeric(auc(thisDS$isInPathway, thisDS$QuackP)),silent = TRUE)
                   try(thisAUC <- roc(thisDS$isInPathway, thisDS$QuackP),silent = TRUE)
...
splitHoldoutForAUC <- function(holdoutFile, model, response, predList){
  ...
  # connected only
  con <- con[,c("isInPathway", "QuackP")]
  con <- con[c('QuackP','isInPathway')]
  con <- con[order(con$QuackP,decreasing=TRUE),] #sort
  
  conAndUnc <- conAndUnc[,c("isInPathway", "QuackP")]
  conAndUnc <- conAndUnc[c('QuackP','isInPathway')]
  conAndUnc <- conAndUnc[order(conAndUnc$QuackP,decreasing=TRUE),] #sort
  
  conAndUncAndUncv <- conAndUncAndUncv[,c("isInPathway", "QuackP")]
  conAndUncAndUncv <- conAndUncAndUncv[c('QuackP','isInPathway')]
